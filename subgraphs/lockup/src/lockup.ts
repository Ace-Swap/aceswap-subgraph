...
import { getAcePrice } from 'pricing'
...
export function getUser(pid: BigInt, address: Address, block: ethereum.Block): User {
  ...
  if (user === null) {
    ...
    user.aceHarvestedSinceLockup = BIG_DECIMAL_ZERO
    user.aceHarvestedSinceLockupUSD = BIG_DECIMAL_ZERO
    user.save()
  }
...
function transfer(pid: BigInt, userAddr: Address, block: ethereum.Block): void {
  ...
  if (block.number.ge(LOCKUP_BLOCK_NUMBER)) {
    ...
    if (pending.gt(BIG_DECIMAL_ZERO)) {
      user.aceHarvestedSinceLockup = user.aceHarvestedSinceLockup.plus(pending)
      const aceHarvestedUSD = pending.times(getAcePrice(block))
      user.aceHarvestedSinceLockupUSD = user.aceHarvestedSinceLockupUSD.plus(aceHarvestedUSD)
    }
  }
